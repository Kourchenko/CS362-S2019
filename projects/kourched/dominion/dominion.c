...
int cardEffectAdventurer(int cardDrawn, int drawntreasure, int currentPlayer, struct gameState *state) {
  int z = 1; // BUG: fail to initialize properly, unsed poition 0
  ...
  while(z-1>=0) { // BUG: temphand[0] will always be empty
...
int cardEffectGreatHall(int handPos, int currentPlayer, struct gameState *state) {
  //+1 Card
  drawCard(currentPlayer, state);
  ...
  for (int i = 0; i < state->numPlayers; i++) {
    if (i != currentPlayer) {
      for (int j = 0; j < state->handCount[i]; j++) {
        if (state->hand[j][i] == copper) { // BUG: user error - switch 'i' and 'j'
...
int cardEffectEmbargo(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state) {
  //+2 Coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
  ...
  switch( card ) 
    {
    case adventurer:
      cardEffectAdventurer(cardDrawn = 0, drawntreasure, currentPlayer, state);
